...
#define COLOR_ARRAY @[\
[UIColor colorWithRed:47/255.0 green:164/255.0 blue:239/255.0 alpha:1],\
[UIColor colorWithRed:250/255.0 green:154/255.0 blue:49/255.0 alpha:1],\
...
-(void)drawRect:(CGRect)rect{
    ...
    centerView.frame = CGRectMake(0, 0, 95, 95);
...
- (void)drawLineWithPointArray:(NSArray *)pointArray centerArray:(NSArray *)centerArray {
    ...
    for (int i = 0; i < pointArray.count; i++) {
        ...
        UIColor *color = KHexRGB(0x999999);
        // 模型数据（绘制数据时要用）
        DVFoodPieModel *model = self.modelArray[i];
        
        // 模型的数据
        NSString *name = model.name;
        NSString *number = [NSString stringWithFormat:@"%d", (int)model.value ];
        ...
        // 指引线终点的位置（x, y）
        CGFloat startX = x + 1 * cos(radianCenter);
        CGFloat startY = y + 1 * sin(radianCenter);
