...
- (NSArray *)getNowDate:(NSDate *)date
{
    ...
    if (self.datePickerStyle == UUDateStyle_YearMonth)
        return @[year,month];
    if (self.datePickerStyle == UUDateStyle_Month)
        return @[month];
    if (self.datePickerStyle == UUDateStyle_Day)
        return @[day];
    return nil;
}
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView
{
    ...
    if (self.datePickerStyle == UUDateStyle_YearMonth){
        if (isIOS7) {
            [self creatValuePointXs:@[@"120",@"200",]
                          withNames:@[@"年",@"月"]];
        }
        return 2;
    }
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component
{
    ...
    if (self.datePickerStyle == UUDateStyle_Day){
        if (component == 0) {
            if(!self.getDayDate)
            {
                self.getDayDate = [NSDate date];
            }
            NSDateFormatter *nextformatter = [[NSDateFormatter alloc] init] ;
            [nextformatter setDateFormat:@"yyyy"];
            NSInteger year=[[nextformatter stringFromDate:self.getDayDate] integerValue];
            [nextformatter setDateFormat:@"MM"];
            NSInteger month=[[nextformatter stringFromDate:self.getDayDate]integerValue];
...
- (CGFloat)pickerView:(UIPickerView *)pickerView widthForComponent:(NSInteger)component
{
    switch (self.datePickerStyle) {
        ...
        case UUDateStyle_YearMonth:{
            ...
            break;
        case UUDateStyle_Month:{
            if (component==0) return 70;
           
        }
            break;
        case UUDateStyle_Day:{
            if (component==0) return 70;
