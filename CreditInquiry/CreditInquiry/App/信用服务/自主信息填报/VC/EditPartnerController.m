...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self setNavigationBarTitle:@"合作伙伴"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            [cell setContent:self.dataDic row:indexPath.row editType:EditTypePartner enable:_canEdit];
            return cell;
        }else{
        ...
        [cell setContent:self.dataDic type:EditTypePartner editable:_canEdit];
        return cell;
    }
}
