...
static NSString * const receiptKey = @"receipt-data";
    ...
    for (SKPaymentTransaction *transaction in transactions) {
        ...
        switch (transaction.transactionState) {
            ...
            case SKPaymentTransactionStatePurchased://交易完成
                ...
                [self completeTransaction:transaction];//结束交易防止刷单
...
//                                   nil];
    ...
    NSDictionary *params = @{@"receipt-data":self.receipt};
   
    [self serverReceiptRequest:params];
}
...
- (void)serverReceiptRequest:(NSDictionary *)params{
    ...
    [manager POST:@"https://sandbox.itunes.apple.com/verifyReceipt" parameters:bodyString progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        NSLog(@"服务端验单数据_______%@",responseObject)
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        NSLog(@"服务端验单error_______%@",error)
    }];
