...
- (void)loadData:(BOOL)loading{
    ...
    [RequestManager postWithURLString:[str stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] parameters:nil success:^(id responseObject) {
        ...
        if ([responseObject[@"result"] intValue] == 0) {
            ...
            NSArray*reponselist = responseObject[@"dataResult"];
            NSArray *models = [OweTaxModel mj_objectArrayWithKeyValuesArray:reponselist];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    OweTaxModel *detailModel = self.datalist[indexPath.section];
    
    NSDictionary* detail = [detailModel mj_keyValues];
